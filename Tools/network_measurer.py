...
class RipeAtlas:    
    ...
    def get_ping_measurement_by_tag(self, tag):
        ...
        dict_target2mfrprbs = {}
        while True:
            res = rt.try_best_request_get(url, 5, "get_ping_measurement_by_tag")
            measurement = json.loads(res.text)
            results = measurement["results"]
    
            for dst in pyprind.prog_bar(results):
                target = dst["target"]
                res_dst = rt.try_best_request_get(dst["result"], 5, "get_ping_measurement_by_tag")
                ...
                for prb in measurement_per_dst:
                    ...
                    for rtt in prb_res:
                        if "rtt" in rtt:
                            ...
                            rtts.append(-1)
                    dict_prb2rtt[prb_id] = {"rtts": rtts, "min": prb["min"]}
                dict_target2mfrprbs[target] = {"measurement": dict_prb2rtt}
...
#   no need for key
    ...
    # 24
    # [{'name': '34759', 'value': 50}, {'name': '20756', 'value': 50}, {'name': '21031', 'value': 50}, {'name': '25224', 'value': 50}, {'name': '24899', 'value': 50}, {'name': '15760', 'value': 50}, {'name': '24908', 'value': 50}, {'name': '32462', 'value': 50}, {'name': '24971', 'value': 50}, {'name': '24633', 'value': 50}, {'name': '35561', 'value': 50}, {'name': '10599', 'value': 50}, {'name': '30352', 'value': 50}, {'name': '11634', 'value': 50}, {'name': '35620', 'value': 50}, {'name': '35415', 'value': 50}, {'name': '12180', 'value': 50}, {'name': '32324', 'value': 50}, {'name': '31304', 'value': 50}, {'name': '12105', 'value': 50}, {'name': '30263', 'value': 50}, {'name': '35001', 'value': 50}, {'name': '26946', 'value': 50}, {'name': '35031', 'value': 50}]
    # {'34759': [-122.0505, 47.5415], '20756': [-114.2805, 48.2205], '21031': [-108.5815, 45.7975], '25224': [-96.8115, 46.8195], '24899': [-84.7785, 45.0415], '15760': [-68.6695, 44.8675], '24908': [-124.4225, 43.0515], '32462': [-113.7895, 42.4985], '24971': [-105.2195, 39.7475], '24633': [-95.8415, 41.2585], '35561': [-82.8095, 39.9975], '10599': [-74.2285, 39.5815], '30352': [-121.9605, 37.2385], '11634': [-113.6215, 37.1195], '35620': [-101.8615, 33.5775], '35415': [-95.9425, 36.1495], '12180': [-86.9385, 36.0575], '32324': [-78.6415, 35.7795], '31304': [-117.1625, 33.5695], '12105': [-110.7525, 32.0795], '30263': [-99.4795, 27.5285], '35001': [-93.2025, 30.1975], '26946': [-84.2805, 30.4405], '35031': [-80.2415, 26.2875]}
...
def ping(hostname, count):
    response = os.system("ping -n %d %s" % (count, hostname))
    if response == 0:
        return True
    else:
        return False
...
def measure_900():
    for ind, account in enumerate(settings.RIPE_ACCOUNT_KEY):
        zone = pytz.country_timezones('us')[0]
        tz = pytz.timezone(zone)
        start_time = datetime.datetime.now(tz).timestamp() + 120

        list_pid = ["34759", "20756", "21031", "25224", "24899", "15760", "24908", "32462", "24971", "24633", "35561",
                    "10599", "30352", "11634", "35620", "35415", "12180", "32324", "31304", "12105", "30263", "35001",
                    "26946", "35031"]

        ripe = RipeAtlas(account["account"], account["key"])
