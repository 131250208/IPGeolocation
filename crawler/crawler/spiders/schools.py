...
class SchoolsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            'https://en.wikipedia.org/wiki/Category:Lists_of_high_schools_in_the_United_States_by_state',
        ]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        list_path = response.css("div.mw-category a::attr(href)").extract()

        for next_sta in list_path:
            url_next = response.urljoin(next_sta)
            search_group = re.search("List_of_high_schools_in_(.*)", url_next)
            yield scrapy.Request(url_next, callback=self.parse_state,
                                 meta={"state_name": re.sub("_", " ", search_group.group(1))})
        ...
        for table in list_table:
            ...
            for row in list_tr[1:]:
                ...
                if len(list_ul_in_td) == 0:
                    ...
                    if uni_name != "NYC DOE":
                        yield {"state_name": state_name, "school_name": uni_name}
                else:
